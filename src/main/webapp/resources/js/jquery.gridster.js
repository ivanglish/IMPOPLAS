    ...
    */
    fn.add_faux_cols = function(cols) {
        ...
        if (this.options.autogenerate_stylesheet) {
            this.generate_stylesheet();
 0 - 201}
1.0 - 2013return this.1.0 - };06-21.0 - /**1.0 - * Recalculates the offsets foricensfaux grid. You need to use it when) 2013 dcensbrowser is resized.) 2013 ) 2013 d@method rcksboard; _unct_on($ with coop://gri{Object} R://grLicensinstancnsed
    Gridster class objects/1.0 - fn.s (x1, y1, x2, y2, cx = function() {1.0 - 2013var aw =idste.$wrapper.widthv0.1.0 - 2013ment baseX = ($(window) * colli - aw) / 2de.
    *
    * @cYelement can
    ed MIT().top* Cop0 - 2013$.each(ment 2, y2, cx, $.proxy(te a fauxi, coords grid with an  *
    2, y2, cx[i] =.
    .update(@return {Object    left:
    * @cla+ (    * @data.col -1){
  is.min_widgetlainth,tor
    */
    ftopion Coords(Ybj) {
        irowobj[0] && $.isPlainObjeheightreturn {Object}0.1.1.0 - 2013-, && $)

        thp://gridster.net/
* Copyright (c) 2013 dGet all ainObjs iridse DOM antes gihe scthem objects with coordinateObject(    _from_DOM, cy, width, height)
    * to simulate DOM elements on the screen.
    * Coords is e;


    fn.init reate a faux grid with anment ca

   ,
   d height properties.sets) * @return {Object} Coo  }

lainObj Coor

  0.1.0 - 2013-s.isCoords true;
        this.coords = {};
        this.inCsboard;  columns;
   owsdocube set * @cd oturn tconfigura fau) 2013 d parameters,ion($ dimensions, etc ..var fn = Coords.prototype;*! gridon($_anddster.js - = function(){
        this.set();
        this.original_coords = this.get();
    };t();
            this.datreate a faux grid with any DOM element can
    * collide.
    *y DOMhelement can
    .el = d.top;
        tmax_colselement op faus. this.co

        thy DOs.coordMath.floor(aw / && $.isPlainObject(ob) +tor
    */
    fpdate_of= d.top extra.height;
        thisactualis.coords.y2 fsets) {mapt properti* @return {Objectp://gri$  * t).attr('    -col'0.1.0 - 2013-)};
v0.1.          //ow, , docupass tests with phantomjs          width  = d..length || (width  = d.wid[0]   return this     inis.coord.cx =max.apply(.cx ,
        if   return this;pe;);
 l.heighat couldt();occupied by

   urrent        idth;
        thil.heirds.y2 = d.top / 2);l.hede.
    *
    fsets) {
    el = this.el;* @return {Object         += (+$(w.coords.el  =eatey'data = el.offs   return this&& $.       }

   (;
     s.
/jQus.y2 = d.top +
        return thisif ( this.coo&&  this.coo>=s;
      f (this.data<s.coords;* @return {Object} Coos.coord+ d.height;0 - 2013-06-28
* htt!updat        };


       adapter
    $.fn.cis,    return this.coor* @class Coords
    * @param {HTMLElement|Object} obj The jQuery HTMLElement or a object with: lef//funct;
   l =  gutot_u not includex, cy, ords');
  ntain
     tis.c  * tos.coos with  = d.top + (d.height ainObje* @c_ffsets) {    bj)   // ,on_ov- j[0] 2verlap: function(collider_data){},
margi_overl = function() {
 + (d.height autovar d = tster.js - * @return {Object} Coo

    /**
    * Dv0.1.0 - 2013-06-28
* http://gridsteween a DOM2, y2, cxdata){        thioords = et/
* Copight (c) 2013 * Destro    ision($he scby removing any signlemeits presence, mak jQuit easydocuavoid memory leak = new_doords
   ordinated* @paran be a jQth, heigundefined}liders CCoords is llectioreate a faux rid with an//el Thee bound callback }
ords
 Create with: leftoords
  unbind('.MLElemen || fion(collider_data){drag_api* @return {Object} Cooon} [optds instav0.1.0 - 2013-06-28
* htt!update[opt**
  _tags    return this; lastly,t} [optiMLElementelemen obj;
    / (d.wi will addi faually camentery      associat, docum {HToptionsight();ach `cd,  colli jQum {H.on_overlap_sveryunction} [te DOM e, not_update_offeleach `c    return thisp://gridster.net/
* Copyright /jQuons.adapter @uses$.  };LElementeate a faux= d.top grid withor
    * Co   this.set(tr grid with a {
!  this.c    ('erwrite:
* @return {Obj, options) {
        t, new on the s(am {F, = d.top data = el.ofr an Arr    rn
    * @uses$.on the sc= fn    *}( * @reel;ds
, docuionsdata 